...
// ---------------------------------------------------------------------------
// Phase 2 marker syntax: [[ comment ]]  and  | bar-lead
// ---------------------------------------------------------------------------

describe('ChoirBox marker syntax: [[ ... ]]', () => {
  it('whole-line [[...]] becomes an italic comment line', () => {
    const cp = `{sov}
[[ ruhig und getragen ]]
Hallo Welt
{eov}`
    ...
    const comment = lines.find((l) => l.isComment)
    expect(comment?.text).toBe('ruhig und getragen')
  })

  it('inline [[...]] becomes an annotation on the lyric line', () => {
    const cp = `Der Mond [[ langsam ]] ist aufgegangen`
    const parsed = parseChordPro(cp)
    const line = parsed.sections[0].lines[0]
    expect(line.annotations).toEqual(['langsam'])
    expect(line.text.includes('[[')).toBe(false)
  })

  it('two inline markers yield two annotations', () => {
    const cp = `[[ p ]] stille [[ f ]] laut`
    const parsed = parseChordPro(cp)
    const line = parsed.sections[0].lines[0]
    expect(line.annotations).toEqual(['p', 'f'])
  })
})

describe('ChoirBox marker syntax: | bar-lead', () => {
  it('leading "| " sets isBarLead on the line', () => {
    const cp = `| Der Mond ist aufgegangen`
    const parsed = parseChordPro(cp)
    const line = parsed.sections[0].lines[0]
    expect(line.isBarLead).toBe(true)
    expect(line.text.trim()).toBe('Der Mond ist aufgegangen')
  })

  it('bar-lead preserves chord positions', () => {
    const cp = `| [Dm]Der [G]Mond`
    const parsed = parseChordPro(cp)
    const line = parsed.sections[0].lines[0]
    expect(line.isBarLead).toBe(true)
    expect(line.chords.map((c) => c.chord)).toEqual(['Dm', 'G'])
    // Mit `| ` ersetzt durch 2 Spaces → Dm sitzt auf Col 2 (ursprünglich "| [Dm]D...")
    expect(line.chords[0].col).toBe(2)
  })

  it('plain pipe inline (no leading space) is NOT a bar-lead', () => {
    const cp = `| [Dm]Der`    // bar-lead
    const cp2 = `[Dm]|Der`     // pipe in middle, no bar-lead
    expect(parseChordPro(cp).sections[0].lines[0].isBarLead).toBe(true)
    expect(parseChordPro(cp2).sections[0].lines[0].isBarLead).toBeUndefined()
  })

  it('does NOT apply inside {sot}...{eot} tab blocks', () => {
    const cp = `{sot}
| x x x |
{eot}`
    const parsed = parseChordPro(cp)
    const line = parsed.sections[0].lines[0]
    expect(line.isBarLead).toBeUndefined()
    expect(line.text).toBe('| x x x |')
  })
})
