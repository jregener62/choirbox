...
- **TXT Schriftgroesse**: Im Fullscreen erscheinen +/- Buttons (rechts ueber Fullscreen-FAB) zum Zoomen der Schrift (7 Stufen, 12px–32px). Text bricht bei jeder Groesse sauber um. Bei `.rtf` werden die +/- Buttons unterdrueckt, sobald das Companion-PDF angezeigt wird (fixe A4-Geometrie) — sie bleiben nur im RtfViewer-Fallback (z.B. waehrend `pdf_status='failed'`) sichtbar
...
**Floating Tag-Toolbar (ueber selektiertem Tag):**

Sobald ein Tag selektiert ist, schwebt eine kleine Mini-Toolbar direkt ueber dem Tag (bei zu wenig Platz oben: darunter). Drei Buttons, je 44×44px Touch-Target:

- ⟨ **Zeichen links** (`ChevronLeft`) — tauscht das Tag mit dem direkt davor stehenden Zeichen. Deaktiviert, wenn das Tag am Textanfang steht.
- ⟩ **Zeichen rechts** (`ChevronRight`) — analog, deaktiviert am Textende.
- 🗑 **Loeschen** (`Trash2`, rot) — entfernt das Tag komplett; Toolbar verschwindet.
...
| `frontend/src/utils/chordProEdit.ts` | `insertAtOffset`, `wrapLinesAsSection`, `findTagAt`, `moveTagLeft`, `moveTagRight`, `deleteTagAt` — pure Helpers fuer Cursor-/Selection-Manipulation |
| `frontend/src/components/ui/TagToolbar.tsx` | Floating Mini-Toolbar ueber selektiertem Tag (Move/Delete) |
...
- **Move-Tool**: In der Annotations-Toolbar gibt es ein Hand-Icon ("Markierung verschieben"). Klick auf eine Markierung selektiert sie (gestricheltes blaues Bounding-Box), Drag verschiebt sie in Echtzeit (SVG-translate), Loslassen schreibt die neuen Punkte zurueck (debounced save). Damit kann der User Markierungen, die nach RTF-Edit verrutscht sind, manuell korrigieren statt sie loeschen zu muessen.
...
- Toolbar-Button (`SeparatorHorizontal`) im RTF-Editor fuegt an der Cursor-Position einen Seitenumbruch ein.
- Tiptap-Custom-Node `pageBreak` (siehe `frontend/src/utils/tiptapPageBreak.ts`) — atomar, selektierbar, mit Backspace loeschbar.
- Round-Trip: Editor `pageBreak` → RTF `\page` → Parser `RtfParagraph.pageBreak: true` → Editor `pageBreak`. Standard-RTF-Steuerwort, externe Editoren (Word/TextEdit) respektieren es ebenfalls.
- **Endlos-Viewer**: zeigt den Bruch als gestrichelte Trennlinie mit "SEITENUMBRUCH"-Label.
- **Seitenansicht**: erzwingt dort eine neue Seite; nachfolgender Inhalt wird automatisch weitergebrochen.
